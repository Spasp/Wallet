...
const initialTranasaction : Transaction ={ id : "Init18",amount:1576.65, date: new Date().toISOString(), title:"Salary June",type:'receive'}
export const BalanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addTransaction = useCallback((transaction: Omit<Transaction, 'id' | 'date'>) => {
    const newTransaction: Transaction = {
     ...
     if (newTransaction.type ==='receive')setBalance(prevBalance => prevBalance + newTransaction.amount);
     if (newTransaction.type ==='send')setBalance(prevBalance => prevBalance - newTransaction.amount);
   
  }, []);

 
 const value = useMemo(()=>({balance, transactions, addTransaction}),[balance])
